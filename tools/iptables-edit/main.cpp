...
int main(int argc, char * argv[])
{
    ...
    try
    {
        // create & run an instance of the editor
        //
        iptables_edit::editor editor(argc, argv);
        return editor.run();
    }
    ...
    {
        SNAP_LOG_FATAL
            << "iptables-edit: libexcept::exception_t caught: "
            << e.what()
            << SNAP_LOG_SEND;
    }
    ...
    {
        SNAP_LOG_FATAL
            << "iptables-edit: std::exception caught: "
            << e.what()
            << SNAP_LOG_SEND;
    }
    catch(...)
    {
        SNAP_LOG_FATAL
            << "iptables-edit: unknown exception caught."
            << SNAP_LOG_SEND;
    }
